...
if ( ! function_exists( 'succulents_qodef_styles' ) ) {
	...
	function succulents_qodef_styles() {
        ...
        $modules_css_deps_array = apply_filters( 'succulents-qodef-modules-css-deps', array() );
		
		//include theme's core styles
		wp_enqueue_style( 'succulents-qodef-default-style', QODE_ROOT . '/style.css' );
		wp_enqueue_style( 'succulents-qodef-modules', QODE_ASSETS_ROOT . '/css/modules.min.css', $modules_css_deps_array );
		...
		if ( succulents_qodef_is_woocommerce_installed() && succulents_qodef_load_woo_assets() ) {
			...
			wp_enqueue_style( 'succulents-qodef-woo', QODE_ASSETS_ROOT . '/css/woocommerce.min.css' );
		}

		if ( succulents_qodef_dashboard_page() ) {
			wp_enqueue_style('succulents-qodef-dashboard', QODE_FRAMEWORK_ADMIN_ASSETS_ROOT. '/css/qodef-dashboard.css');
		}
		...
		if ( succulents_qodef_is_responsive_on() ) {
			wp_enqueue_style( 'succulents-qodef-modules-responsive', QODE_ASSETS_ROOT . '/css/modules-responsive.min.css' );
			
			//is woocommerce installed?
			if ( succulents_qodef_is_woocommerce_installed() && succulents_qodef_load_woo_assets() ) {
				//include theme's woocommerce responsive styles
				wp_enqueue_style( 'succulents-qodef-woo-responsive', QODE_ASSETS_ROOT . '/css/woocommerce-responsive.min.css' );
			}
...
if ( ! function_exists( 'succulents_qodef_google_fonts_styles' ) ) {
	...
	function succulents_qodef_google_fonts_styles() {
		...
		if ( count( $fonts_array ) > 0 ) {
			...
			wp_enqueue_style( 'succulents-qodef-google-fonts', esc_url_raw( $succulents_qodef_fonts ), array(), '1.0.0' );
...
if ( ! function_exists( 'succulents_qodef_scripts' ) ) {
	...
	function succulents_qodef_scripts() {
		...
		if ( ! empty( $google_maps_api_key ) ) {
			wp_enqueue_script( 'succulents-qodef-google-map-api', '//maps.googleapis.com/maps/api/js?key=' . esc_attr( $google_maps_api_key ) . $google_maps_extensions, array(), false, true );
		}
		
		wp_enqueue_script( 'succulents-qodef-modules', QODE_ASSETS_ROOT . '/js/modules.min.js', array( 'jquery' ), false, true );
		
		if ( succulents_qodef_dashboard_page() ) {
			wp_enqueue_script('succulents-qodef-dashboard', QODE_FRAMEWORK_ADMIN_ASSETS_ROOT. '/js/qodef-dashboard.js');
...
if ( ! function_exists( 'succulents_qodef_enqueue_editor_customizer_styles' ) ) {
	/**
	 * Enqueue supplemental block editor styles
	 */
	function succulents_qodef_enqueue_editor_customizer_styles() {
		wp_enqueue_style( 'succulents-qodef-style-modules-admin-styles', QODE_FRAMEWORK_ADMIN_ASSETS_ROOT . '/css/qodef-modules-admin.css' );
		wp_enqueue_style( 'succulents-qodef-style-handle-editor-customizer-styles', QODE_FRAMEWORK_ADMIN_ASSETS_ROOT . '/css/editor-customizer-style.css' );
	}
	
	// add google font
	add_action( 'enqueue_block_editor_assets', 'succulents_qodef_google_fonts_styles' );
	// add action
	add_action( 'enqueue_block_editor_assets', 'succulents_qodef_enqueue_editor_customizer_styles' );
}
...
if ( ! function_exists( 'succulents_qodef_page_custom_style' ) ) {
	...
	function succulents_qodef_page_custom_style() {
		...
		if ( $style !== '' ) {
			...
			if ( succulents_qodef_is_woocommerce_installed() && succulents_qodef_load_woo_assets() ) {
				wp_add_inline_style( 'succulents-qodef-woo', $style );
			} else {
				wp_add_inline_style( 'succulents-qodef-modules', $style );
			}
		}
	}
...
if ( ! function_exists( 'succulents_qodef_print_custom_js' ) ) {
	...
	function succulents_qodef_print_custom_js() {
		...
		if ( ! empty( $custom_js ) ) {
			wp_add_inline_script( 'succulents-qodef-modules', $custom_js );
		}
	}
...
if ( ! function_exists( 'succulents_qodef_get_global_variables' ) ) {
	...
	function succulents_qodef_get_global_variables() {
		...
		wp_localize_script( 'succulents-qodef-modules', 'qodefGlobalVars', array(
...
if ( ! function_exists( 'succulents_qodef_per_page_js_variables' ) ) {
	...
	function succulents_qodef_per_page_js_variables() {
		...
		wp_localize_script( 'succulents-qodef-modules', 'qodefPerPageVars', array(
...
if ( ! function_exists( 'succulents_qodef_is_wp_gutenberg_installed' ) ) {
	/**
	 * Function that checks if WordPress 5.x with Gutenberg editor installed
	 * @return bool
	 */
	function succulents_qodef_is_wp_gutenberg_installed() {
		return class_exists( 'WP_Block_Type' );
	}
}

if ( ! function_exists('succulents_qodef_get_module_part') ) {
	function succulents_qodef_get_module_part($module ) {
		return $module;
	}
}
